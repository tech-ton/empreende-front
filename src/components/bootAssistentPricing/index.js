...
export default function BootAssistentPricing () {
  ...
  async function mainBoot(userMessage) {
    try{
      const result = await model.generateContent(`Quero que você responda somente assunto da realização de uma precificação focado no inicio de negócio com os topicos: Custos Fixos e Variáveis. Qualquer assunto distinto corrija para retornar sobre o assunto e edite o texto da resposta para cada linha ter no máximo 120 caracteres e se ultrapassar use quebra de linha. O seguimento da minha loja é de ` +userBusiness.negocio+" e modelo "+userDataBusiness.tipoNegocio+" "+userMessage);
