...
import dataChat from "../../data/chat-data-precificacao.json"
...
export default function BootAssistentPricing () {
  ...
  useEffect(() => {
      const storedItems = JSON.parse(localStorage.getItem('userChatPricing')) || dataChat;
      setChat(storedItems);
    }, []);
  ...
  async function mainBoot(userMessage) {
    try{
      const result = await model.generateContent(`Quero que você responda somente assunto da realização de uma precificação focado no inicio de negócio com os topicos: Custos Fixos e Variáveis. O seguimento da minha loja é de ` +userBusiness.negocio+" e modelo "+userDataBusiness.tipoNegocio+" "+userMessage);
      ...
      localStorage.setItem('userChatPricing', JSON.stringify(addedItens));
    }catch (error) {
